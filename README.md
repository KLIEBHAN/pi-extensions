...
- `/prompt-autocomplete status` reports the current session's issued requests, requests served from the cache, tokens, and estimated cost. Token counts come from the provider, but the cost is derived locally from pi's model price table and is therefore an estimate, not an invoice. A trailing `+` means at least one response reported nothing, so the real total is higher.
