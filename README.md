...
- The reviewer also returns a structured `## Review Data` JSON block with `schemaVersion: 1`, which the extension uses for robust checklist rendering; invalid structured data is surfaced and falls back to Markdown parsing. Reviews without a recognized `APPROVE`, `APPROVE_WITH_NOTES`, or `CHANGES_REQUESTED` verdict fail closed and can be retried. When the reviewer subprocess exits cleanly but returns no assistant text at all, the failure message says so explicitly and includes reviewer diagnostics (`stopReason` and a short `stderr` tail) so reasoning-only/length/provider issues are easy to spot; these diagnostics are also written to the run artifact.
