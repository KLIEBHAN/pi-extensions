...
test("agent_end still continues when exactly one adjacent slot remains", async () => {
  const { createAutoModeExtension } = await loadAutoModeModule();
  const harness = createHarness();
  let adjacentDecisionCalls = 0;
  ...
  harness.entries.push({
    ...
    data: {
      ...
      enabled: true,
      paused: false,
      runId: "auto-restored-adjacent-one-slot-left",
  ...
  createAutoModeExtension({
    getGitSnapshot: async () => ({
      isGitRepo: true,
      head: "head-restored-adjacent-one-slot-left",
      status: "## main",
      changedFiles: [],
      dirty: false,
      hasUpstream: false,
      repoFingerprint: "clean-fingerprint-restored-adjacent-one-slot-left",
    }),
    decideControllerAction: async () => ({
      action: "stop",
